...
function getModelsByCategory(category) {
    return exports.MODEL_REGISTRY.filter(model => model.category === category);
}
// Get fallback model for a category
function getFallbackModel(category, excludeId) {
    const models = exports.MODEL_REGISTRY.filter(m => m.category === category && m.id !== excludeId);
    return models[1] || models[0]; // Return second or first as fallback
}
// Get model by ID
function getModelById(id) {
    return exports.MODEL_REGISTRY.find(model => model.id === id);
}
